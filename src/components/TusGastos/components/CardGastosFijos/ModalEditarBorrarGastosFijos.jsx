  ...
  function ModalEditarBorrarGastosFijos({
    ...
    useEffect(() => {
      if (isOpen && userId && gastoFijoId) {
        const fetchUrl = `https://finanplusback.zeabur.app/usuarios/${userId}/gastosfijos/${gastoFijoId}/gastos`;
        fetch(fetchUrl)
    ...
    const handleDeleteGasto = () => {
      if (gastoSeleccionado && gastoSeleccionado.gastoID) {
        const url = `https://finanplusback.zeabur.app/usuarios/${userId}/gastosfijos/${gastoFijoId}/gastos/${gastoSeleccionado.gastoID}`;
        fetch(url, {
          method: "DELETE",
        })
          .then((response) => {
