  ...
  function ModalEditarBorrarGastos({
    ...
    useEffect(() => {
      if (isOpen && userId && gastoDiarioId) {
        const fetchUrl = `https://finanplusback.zeabur.app/usuarios/${userId}/gastosdiario/${gastoDiarioId}/gastos`;
        fetch(fetchUrl)
    ...
    const handleDeleteGasto = () => {
      if (gastoSeleccionado && gastoSeleccionado.gastoID) {
        const url = `https://finanplusback.zeabur.app/usuarios/${userId}/gastosdiario/${gastoDiarioId}/gastos/${gastoSeleccionado.gastoID}`;
        fetch(url, {
          method: "DELETE",
        })
          .then((response) => {
