  ...
  function ModalEditarBorrarGastoscc({
    ...
    useEffect(() => {
      if (isOpen && userId && tarjetaCreditoID) {
        const fetchUrl = `https://finanplusback.zeabur.app/usuarios/${userId}/tarjetascredito/${tarjetaCreditoID}/gastos`;
        fetch(fetchUrl)
    ...
    const handleDeleteGasto = () => {
      if (gastoSeleccionado && gastoSeleccionado.gastoID) {
        const url = `https://finanplusback.zeabur.app/usuarios/${userId}/tarjetascredito/${tarjetaCreditoID}/gastos/${gastoSeleccionado.gastoID}`;
        fetch(url, {
          method: "DELETE",
        })
          .then((response) => {
