...
function TusIngresos({ userId, currentDate }) {
  ...
  const obtenerIngresos = () => {
    if (userId) {
      fetch(`https://finanplusback.zeabur.app/usuarios/${userId}/ingresos`)
        .then((response) => {
          if (!response.ok) {
  ...
  useEffect(() => {
    if (userId && currentDate) {
      ...
      const url = `https://finanplusback.zeabur.app/usuarios/${userId}/ingresos/fecha?year=${year}&month=${month}`;
      fetch(url)
  ...
  const handleSubmit = () => {
    ...
    const url = `https://finanplusback.zeabur.app/usuarios/${userId}/ingresos`;
    const body = {
